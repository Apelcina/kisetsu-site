...
`,G=class{shader;vao;vertexCount;gl;constructor(e,t){this.gl=e,this.shader=new N(e,U,W),this.vertexCount=t.vertexCount,this.vao=e.createVertexArray();let n=e.createBuffer();e.bindVertexArray(this.vao),e.bindBuffer(e.ARRAY_BUFFER,n),e.bufferData(e.ARRAY_BUFFER,t.interleaved,e.STATIC_DRAW),e.enableVertexAttribArray(0),e.vertexAttribPointer(0,3,e.FLOAT,!1,36,0),e.enableVertexAttribArray(1),e.vertexAttribPointer(1,3,e.FLOAT,!1,36,12),e.enableVertexAttribArray(2),e.vertexAttribPointer(2,3,e.FLOAT,!1,36,24),e.bindVertexArray(null)}draw(e,t,n,r,i){let a=this.gl;this.shader.use(),this.shader.setMat4(`model`,e),this.shader.setMat4(`view`,t),this.shader.setMat4(`projection`,n),this.shader.setVec3(`lightPos`,r),this.shader.setVec3(`lightColor`,y(1,1,1)),this.shader.setVec3(`viewPos`,i),a.bindVertexArray(this.vao),a.drawArrays(a.TRIANGLES,0,this.vertexCount),a.bindVertexArray(null)}};function K(e){let t={},n=``;for(let r of e.split(`
`)){let e=r.trim();if(e.startsWith(`newmtl `))n=e.slice(7).trim();else if(e.startsWith(`Kd `)&&n){let[,r,i,a]=e.split(/\s+/);t[n]={r:parseFloat(r),g:parseFloat(i),b:parseFloat(a)}}}return t}async function te(e,t){let n=await fetch(e).then(e=>e.text()),r={},i=n.match(/^mtllib (.+)$/m),a=t??(i?e.replace(/[^/]+$/,i[1].trim()):null);a&&(r=K(await fetch(a).then(e=>e.text()).catch(()=>``)));let o=[],s=[],c=[],l={r:.7,g:.75,b:.8};for(let e of n.split(`
`)){let t=e.trim();if(t.startsWith(`v `)){let[,e,n,r]=t.split(/\s+/);o.push(parseFloat(e),parseFloat(n),parseFloat(r))}else if(t.startsWith(`vn `)){let[,e,n,r]=t.split(/\s+/);s.push(parseFloat(e),parseFloat(n),parseFloat(r))}else if(t.startsWith(`usemtl `)){let e=t.slice(7).trim();l=r[e]??l}else if(t.startsWith(`f `)){let e=t.split(/\s+/).slice(1);for(let t=1;t<e.length-1;t++)for(let n of[e[0],e[t],e[t+1]]){let e=n.split(`/`),t=(parseInt(e[0])-1)*3;if(c.push(o[t],o[t+1],o[t+2]),e[2]&&e[2]!==``){let t=(parseInt(e[2])-1)*3;c.push(s[t],s[t+1],s[t+2])}else c.push(0,1,0);c.push(l.r,l.g,l.b)}}}return{interleaved:new Float32Array(c),vertexCount:c.length/9}}var ne=class{keys=new Set;orbitAzimuth=0;orbitElevation=15;orbitDistance=30;mouseDown=!1;lastMouseX=0;lastMouseY=0;constructor(e){window.addEventListener(`keydown`,e=>this.keys.add(e.code)),window.addEventListener(`keyup`,e=>this.keys.delete(e.code)),e.addEventListener(`mousedown`,e=>{e.button===0&&(this.mouseDown=!0,this.lastMouseX=e.clientX,this.lastMouseY=e.clientY)}),window.addEventListener(`mouseup`,e=>{e.button===0&&(this.mouseDown=!1)}),window.addEventListener(`mousemove`,e=>{if(!this.mouseDown)return;let t=e.clientX-this.lastMouseX,n=e.clientY-this.lastMouseY;this.lastMouseX=e.clientX,this.lastMouseY=e.clientY,this.orbitAzimuth-=t*.15,this.orbitElevation=Math.max(-80,Math.min(80,this.orbitElevation+n*.15))}),e.addEventListener(`wheel`,e=>{e.preventDefault(),this.orbitDistance=Math.max(3,Math.min(50,this.orbitDistance+e.deltaY*.02))},{passive:!1})}isOrbiting(){return this.mouseDown}held(e){return document.activeElement?.tagName===`INPUT`?!1:this.keys.has(e)}};function re(){return{shields:20,maxShields:100,health:10,maxHealth:10,engines:30,maxEngines:30,energy:100,maxEnergy:100,pulse:{name:`PULSE`,auto:!1,reloading:!1,reloadTimer:0,reloadTime:4.5,shotsPerBurst:1,pendingFire:!1},lance:{name:`LANCE`,auto:!1,reloading:!1,reloadTimer:0,reloadTime:6,shotsPerBurst:1,pendingFire:!1},hitRing:0,hitPos:y(0,0,1),chargeWave:-1,lasers:{name:`LASERS`,auto:!1,reloading:!1,reloadTimer:0,reloadTime:3,shotsPerBurst:2,pendingFire:!1}}}var ie=class{el;constructor(e){this.el=document.createElement(`div`),this.el.id=`hud`,this.el.innerHTML=`
      <div class="hud-panel">
        <div class="hud-row">
    ...
    `,document.head.appendChild(e)}},oe=class{el;lockedEnemy=null;orbiting=!1;orbitBtn;rows=[];onLockChange;onOrbitToggle;constructor(e,t){this.onLockChange=e,this.onOrbitToggle=t,this.el=document.createElement(`div`),this.el.id=`target-hud`,this.el.innerHTML=`
      <div class="tgt-panel">
        ...
        <div id="tgt-rows"></div>
      </div>`,document.body.appendChild(this.el),this.orbitBtn=document.getElementById(`tgt-orbit-btn`),this.orbitBtn.addEventListener(`click`,()=>{this.lockedEnemy&&(this.setOrbiting(!this.orbiting),this.onOrbitToggle(this.orbiting))}),this.injectStyles()}update(e){let t=e.filter(e=>e.dist<150);if(this.lockedEnemy&&!t.find(e=>e.obj===this.lockedEnemy)&&(this.lockedEnemy=null,this.setOrbiting(!1),this.onLockChange(!1,null),this.onOrbitToggle(!1)),this.orbitBtn.style.visibility=this.lockedEnemy?`visible`:`hidden`,t.length===0){this.el.style.display=`none`;return}this.el.style.display=`block`;let n=document.getElementById(`tgt-rows`);for(;this.rows.length<t.length;){let e=document.createElement(`div`);e.className=`tgt-row`;let t=document.createElement(`div`);t.className=`tgt-info`;let r=document.createElement(`span`);r.className=`tgt-key`,r.textContent=`SHD`;let i=document.createElement(`span`),a=document.createElement(`span`);a.className=`tgt-dist`;let o=document.createElement(`button`);o.className=`tgt-lock-btn`,t.append(r,i,a);let s=document.createElement(`div`);s.className=`tgt-bars`;let c=document.createElement(`div`);c.className=`tgt-health-bar`;let l=[];for(let e=0;e<10;e++){let e=document.createElement(`div`);e.className=`tgt-health-block`,c.appendChild(e),l.push(e)}let u=document.createElement(`div`);u.className=`tgt-shield-bar`;let d=[];for(let e=0;e<5;e++){let e=document.createElement(`div`);e.className=`tgt-shield-circle`,u.appendChild(e),d.push(e)}s.append(c,u),e.append(t,s,o),n.appendChild(e);let f={el:e,shieldsEl:i,distEl:a,btn:o,healthBlocks:l,shieldCircles:d,enemy:null};o.addEventListener(`click`,()=>{this.lockedEnemy===f.enemy?(this.lockedEnemy=null,this.orbiting=!1,this.onLockChange(!1,null),this.onOrbitToggle(!1)):(this.lockedEnemy=f.enemy,this.orbitBtn.className=`tgt-orbit-btn available`,this.onLockChange(!0,f.enemy))}),this.rows.push(f)}this.rows.forEach((e,n)=>e.el.style.display=n<t.length?`flex`:`none`),t.forEach((e,t)=>{let n=this.rows[t];n.enemy=e.obj,n.shieldsEl.textContent=`${Math.round(e.shields)}`,n.distEl.textContent=`${e.dist.toFixed(0)}u`;let r=Math.max(0,e.health);n.healthBlocks.forEach((e,t)=>{e.className=`tgt-health-block${t>=r?` empty`:``}`}),n.shieldCircles.forEach((t,n)=>{let r=n*20;e.shields>=r+20?t.className=`tgt-shield-circle full`:e.shields>=r+10?t.className=`tgt-shield-circle half`:t.className=`tgt-shield-circle`});let i=e.obj===this.lockedEnemy;n.btn.textContent=`⊕`,n.btn.className=`tgt-lock-btn${i?` locked`:``}`})}clearLock(){this.lockedEnemy=null,this.setOrbiting(!1)}setOrbiting(e){this.orbiting=e,this.lockedEnemy?this.orbitBtn.className=e?`tgt-orbit-btn orbiting`:`tgt-orbit-btn available`:this.orbitBtn.className=`tgt-orbit-btn`}injectStyles(){let e=document.createElement(`style`);e.textContent=`
      #target-hud {
    ...
    `,document.head.appendChild(e)}},de=144,fe=120,pe=2,me=10,he=72,ge=120,_e=2.5,ve=20;function q(e,t){let n=E(g(),k(g(),t,e));return{pos:_(e),dir:n,traveled:0,dead:!1}}function ye(e,t,n,r){for(let i of e){if(i.dead)continue;let e=de*t;if(w(i.pos,i.pos,i.dir,e),i.traveled+=e,i.traveled>fe){i.dead=!0;continue}T(i.pos,n)<pe&&(r(me,_(i.pos)),i.dead=!0)}for(let t=e.length-1;t>=0;t--)e[t].dead&&e.splice(t,1)}function be(e,t){let n=E(g(),k(g(),t,e));return{pos:_(e),dir:n,traveled:0,dead:!1}}function xe(e,t,n,r){for(let i of e){if(i.dead)continue;let e=he*t;if(w(i.pos,i.pos,i.dir,e),i.traveled+=e,i.traveled>ge){i.dead=!0;continue}T(i.pos,n)<_e&&(r(ve,_(i.pos)),i.dead=!0)}for(let t=e.length-1;t>=0;t--)e[t].dead&&e.splice(t,1)}var J=40,Y=2,Se=12,Ce=14,we=55,Te=6,Ee=3.6,De=60,Oe=2.2,ke=144,Ae=class{pos=y(20,4,-20);heading=y(-1,0,0);shields=80;maxShields=80;health=10;maxHealth=10;hitRing=0;hitPos=y(0,0,1);chargeWave=-1;lasers=[];prevShieldBubble=0;target=g();yaw=180;pitch=0;fireTimer=Math.random()*Oe;constructor(e){e&&b(this.pos,e),this.pickWaypoint()}pickWaypoint(){this.target=y((Math.random()*2-1)*J,Y+Math.random()*(Se-Y),(Math.random()*2-1)*J)}update(e,t){let n=k(g(),this.target,this.pos);v(n)<Te&&this.pickWaypoint();let i=Math.atan2(n[0],n[2])*180/Math.PI,a=Math.atan2(n[1],Math.sqrt(n[0]**2+n[2]**2))*180/Math.PI,o=((i-this.yaw)%360+540)%360-180,s=a-this.pitch,c=we*e;o=Math.max(-c,Math.min(c,o)),s=Math.max(-c,Math.min(c,s)),this.yaw+=o,this.pitch+=s;let l=r(this.yaw),u=r(this.pitch);if(this.heading=E(g(),y(Math.sin(l)*Math.cos(u),Math.sin(u),Math.cos(l)*Math.cos(u))),w(this.pos,this.pos,this.heading,Ce*e),this.pos[0]=Math.max(-40,Math.min(J,this.pos[0])),this.pos[1]=Math.max(Y,Math.min(Se,this.pos[1])),this.pos[2]=Math.max(-40,Math.min(J,this.pos[2])),t){let n=T(this.pos,t);this.fireTimer-=e,this.fireTimer<=0&&n<De&&(this.lasers.push(q(this.pos,t)),this.fireTimer=Oe)}for(let t of this.lasers){if(t.dead)continue;let n=ke*e;w(t.pos,t.pos,t.dir,n),t.traveled+=n,t.traveled>120&&(t.dead=!0)}for(let e=this.lasers.length-1;e>=0;e--)this.lasers[e].dead&&this.lasers.splice(e,1);this.shields=Math.min(this.maxShields,this.shields+Ee*e);let d=Math.floor(this.shields/20);d>this.prevShieldBubble&&(this.chargeWave=0),this.prevShieldBubble=d,this.chargeWave>=0&&(this.chargeWave+=e*2.5,this.chargeWave>=1&&(this.chargeWave=-1)),this.hitRing>0&&(this.hitRing+=e*2.5,this.hitRing>=1&&(this.hitRing=0))}get modelMatrix(){let e=i();return s(e,e,this.pos),u(e,e,r(this.yaw)),l(e,e,r(-this.pitch)),e}},je=`#version 300 es
layout (location = 0) in vec3 aPos;
...
`,Ne=3,X=.06,Pe=1.4,Fe=.18,Ie=class{shader;vao;indexCount;gl;constructor(e){this.gl=e,this.shader=new N(e,je,Me);let{vao:t,indexCount:n}=this.buildQuad();this.vao=t,this.indexCount=n}buildQuad(){let e=this.gl,t=X,n=new Float32Array([-.06,-.06,0,t,-.06,0,t,t,0,-.06,t,0,-.06,-.06,1,t,-.06,1,t,t,1,-.06,t,1]),r=new Uint16Array([0,1,2,0,2,3,4,5,6,4,6,7,0,1,5,0,5,4,1,2,6,1,6,5,2,3,7,2,7,6,3,0,4,3,4,7]),i=e.createVertexArray(),a=e.createBuffer(),o=e.createBuffer();return e.bindVertexArray(i),e.bindBuffer(e.ARRAY_BUFFER,a),e.bufferData(e.ARRAY_BUFFER,n,e.STATIC_DRAW),e.bindBuffer(e.ELEMENT_ARRAY_BUFFER,o),e.bufferData(e.ELEMENT_ARRAY_BUFFER,r,e.STATIC_DRAW),e.enableVertexAttribArray(0),e.vertexAttribPointer(0,3,e.FLOAT,!1,12,0),e.bindVertexArray(null),{vao:i,indexCount:r.length}}draw(e,t,n,r=y(.2,1,1),a=Ne,o=X){if(e.length===0)return;let l=this.gl;this.shader.use(),this.shader.setMat4(`view`,t),this.shader.setMat4(`projection`,n),this.shader.setVec3(`laserColor`,r),l.enable(l.BLEND),l.blendFunc(l.ONE,l.ONE),l.depthMask(!1),l.bindVertexArray(this.vao);for(let t of e){let e=i();s(e,e,w(g(),t.pos,t.dir,-a));let n=Math.abs(t.dir[1])<.99?y(0,1,0):y(1,0,0),r=E(g(),D(g(),n,t.dir)),u=D(g(),t.dir,r);e[0]=r[0],e[1]=r[1],e[2]=r[2],e[4]=u[0],e[5]=u[1],e[6]=u[2],e[8]=t.dir[0],e[9]=t.dir[1],e[10]=t.dir[2],c(e,e,[o/X,o/X,a]),this.shader.setMat4(`model`,e),l.drawElements(l.TRIANGLES,this.indexCount,l.UNSIGNED_SHORT,0)}l.bindVertexArray(null),l.depthMask(!0),l.disable(l.BLEND)}get pulseLength(){return Pe}get pulseWidth(){return Fe}get pulseColor(){return y(1,.5,.05)}},Le=`#version 300 es
layout (location = 0) in vec3 aPos;
...
`,ze=class{shader;vao;gl;constructor(e){this.gl=e,this.shader=new N(e,Le,Re),this.vao=this.buildQuad()}buildQuad(){let e=this.gl,t=new Float32Array([-1,0,0,1,0,0,1,0,1,-1,0,1]),n=new Uint16Array([0,1,2,0,2,3]),r=e.createVertexArray(),i=e.createBuffer(),a=e.createBuffer();return e.bindVertexArray(r),e.bindBuffer(e.ARRAY_BUFFER,i),e.bufferData(e.ARRAY_BUFFER,t,e.STATIC_DRAW),e.bindBuffer(e.ELEMENT_ARRAY_BUFFER,a),e.bufferData(e.ELEMENT_ARRAY_BUFFER,n,e.STATIC_DRAW),e.enableVertexAttribArray(0),e.vertexAttribPointer(0,3,e.FLOAT,!1,12,0),e.bindVertexArray(null),r}draw(e,t,n,r,i,a,o){let s=this.gl;this.shader.use(),this.shader.setMat4(`view`,e),this.shader.setMat4(`projection`,t),this.shader.setVec3(`startPos`,n),this.shader.setVec3(`endPos`,r),this.shader.setVec3(`viewPos`,i),this.shader.setFloat(`width`,.04),this.shader.setVec3(`beamColor`,y(1,.05,.05)),this.shader.setFloat(`alpha`,a/o*.55),s.enable(s.BLEND),s.blendFunc(s.SRC_ALPHA,s.ONE_MINUS_SRC_ALPHA),s.depthMask(!1),s.bindVertexArray(this.vao),s.drawElements(s.TRIANGLES,6,s.UNSIGNED_SHORT,0),s.bindVertexArray(null),s.depthMask(!0),s.disable(s.BLEND)}},Z=36,Be=72,Q=90,Ve=.08,He=35,Ue=.07,We=class{gl;width;height;grid;shield;engine;ship;hud;playerStatus;weaponsHud;targetHud;cameraLocked=!1;orbitActive=!1;orbitRadius=20;smoothCamPos=g();smoothCamTarget=g();camInitialized=!1;wasOrbiting=!1;ready=!1;startTime=performance.now();shipPos=y(0,0,0);shipFront=y(0,0,-1);worldUp=y(0,1,0);yaw=-90;pitch=0;currentSpeed=Z*.03;rollAngle=0;pitchAngle=0;shipState;input;lightPos=y(10,10,10);enemies=(()=>{let e=new Ae,t=new Ae(y(-20,4,20));return t.shields=20,t.maxShields=20,[e,t]})();lockedEnemy=null;lastTime=performance.now();enemyShieldColor=y(1,.45,0);laserRenderer;beamRenderer;lasers=[];burstShotsLeft=0;burstCooldown=0;SHOT_INTERVAL=.15;pulses=[];pulseShotsLeft=0;pulseCooldown=0;lanceTimeLeft=0;LANCE_DURATION=1;lanceEndOffset=g();prevPlayerBubble=0;targetIndicator;constructor(e,t,n,r){this.gl=e,this.width=t,this.height=n,this.shipState=re(),this.input=new ne(r),this.createTargetIndicator(),this.init()}async init(){try{let e=this.gl,t=await te(`/games/yes-captain/models/craft_speederA.obj`);this.grid=new I(e),this.shield=new z(e),this.engine=new H(e),this.ship=new G(e,t),this.playerStatus=new se(this.shipState),this.hud=new ie(this.shipState),this.weaponsHud=new ae(this.shipState,()=>this.fireBurst(),()=>this.firePulseBurst(),()=>this.fireLance()),this.targetHud=new oe((e,t)=>{this.cameraLocked=e,this.lockedEnemy=e?t:null,e||(this.orbitActive=!1)},e=>{this.orbitActive=e,e&&(this.orbitRadius=30)}),new ue(this.shipState),this.laserRenderer=new Ie(e),this.beamRenderer=new ze(e),this.ready=!0}catch(e){console.error(`Renderer init failed:`,e)}}resize(e,t){this.width=e,this.height=t,this.gl.viewport(0,0,e,t)}frame(){if(!this.ready)return;let e=this.gl,t=performance.now(),n=Math.min((t-this.lastTime)/1e3,.05);if(this.lastTime=t,this.processInput(n),!this.input.isOrbiting()){let e=Math.atan2(-this.shipFront[0],-this.shipFront[2])*180/Math.PI,t=.08*n*60,r=((e-this.input.orbitAzimuth)%360+540)%360-180;this.input.orbitAzimuth+=r*t,this.input.orbitElevation+=(15-this.input.orbitElevation)*t}let a=r(this.input.orbitAzimuth),o=r(this.input.orbitElevation),f=this.input.orbitDistance,h=y(this.shipPos[0]+f*Math.cos(o)*Math.sin(a),this.shipPos[1]+f*Math.sin(o),this.shipPos[2]+f*Math.cos(o)*Math.cos(a)),_,x;if(this.cameraLocked&&this.lockedEnemy)if(x=this.lockedEnemy.pos,this.input.isOrbiting()){if(!this.wasOrbiting){let e=k(g(),this.smoothCamPos,this.shipPos),t=Math.sqrt(e[0]**2+e[2]**2);this.input.orbitElevation=Math.atan2(e[1],t)*180/Math.PI,this.input.orbitAzimuth=Math.atan2(e[0],e[2])*180/Math.PI,this.input.orbitDistance=v(e)}_=h,x=this.shipPos}else{let e=E(g(),k(g(),this.lockedEnemy.pos,this.shipPos)),t=ee(g(),e,-f);t[1]+=f*.25,_=S(g(),this.shipPos,t)}else _=h,x=this.shipPos;this.camInitialized||=(b(this.smoothCamPos,_),b(this.smoothCamTarget,x),!0);let C=.06*n*60,w=this.cameraLocked&&this.input.isOrbiting()?.03*n*60:.06*n*60;O(this.smoothCamPos,this.smoothCamPos,_,C),O(this.smoothCamTarget,this.smoothCamTarget,x,w);let D=m(i(),this.smoothCamPos,this.smoothCamTarget,this.worldUp),N=p(i(),r(45),this.width/this.height,.1,1e3);e.viewport(0,0,this.width,this.height),e.clear(e.COLOR_BUFFER_BIT|e.DEPTH_BUFFER_BIT),this.grid.draw(D,N);let P=90-this.yaw,F=i();s(F,F,this.shipPos),u(F,F,r(P)),l(F,F,r(this.pitchAngle)),d(F,F,r(this.rollAngle)),this.ship.draw(F,D,N,this.lightPos,this.smoothCamPos);let I=i();u(I,I,r(P)),l(I,I,r(this.pitchAngle)),d(I,I,r(this.rollAngle));let L=M(A(),j(0,.3,0,0),I),R=S(g(),this.shipPos,y(L[0],L[1],L[2])),z=i();s(z,z,R),c(z,z,[1.5,1.5,1.5]),this.shield.draw(z,D,N,this.smoothCamPos,this.shipState.shields,void 0,this.shipState.hitRing,this.shipState.hitPos,this.shipState.chargeWave);let B=(performance.now()-this.startTime)/1e3;for(let e of[-.35,.35]){let t=j(e,.25,-.9,0),n=M(A(),t,I),r=S(g(),this.shipPos,y(n[0],n[1],n[2])),i=this.currentSpeed/Z*100;this.engine.draw(r,I,D,N,i,B)}this.weaponsHud.update(n,this.cameraLocked,this.shipState.energy/this.shipState.maxEnergy),this.shipState.lasers.pendingFire&&!this.shipState.lasers.reloading&&this.burstShotsLeft===0&&(this.shipState.lasers.pendingFire=!1,this.cameraLocked&&this.fireBurst()),this.burstShotsLeft>0&&(this.burstCooldown-=n,this.burstCooldown<=0&&(this.lockedEnemy&&this.lasers.push(q(this.shipPos,this.lockedEnemy.pos)),this.shipState.energy=Math.max(0,this.shipState.energy-3),this.burstShotsLeft--,this.burstCooldown=this.SHOT_INTERVAL,this.burstShotsLeft===0&&(this.shipState.lasers.reloading=!0,this.shipState.lasers.reloadTimer=this.shipState.lasers.reloadTime))),this.shipState.pulse.pendingFire&&!this.shipState.pulse.reloading&&this.pulseShotsLeft===0&&(this.shipState.pulse.pendingFire=!1,this.cameraLocked&&this.firePulseBurst()),this.pulseShotsLeft>0&&(this.pulseCooldown-=n,this.pulseCooldown<=0&&(this.lockedEnemy&&this.pulses.push(be(this.shipPos,this.lockedEnemy.pos)),this.shipState.energy=Math.max(0,this.shipState.energy-8),this.pulseShotsLeft--,this.pulseCooldown=.3,this.pulseShotsLeft===0&&(this.shipState.pulse.reloading=!0,this.shipState.pulse.reloadTimer=this.shipState.pulse.reloadTime)));let V=(e,t)=>{if(this.lockedEnemy){if(this.lockedEnemy.shields>=20){this.lockedEnemy.shields=Math.max(0,this.lockedEnemy.shields-e);let n=k(g(),t,this.lockedEnemy.pos);E(this.lockedEnemy.hitPos,n),this.lockedEnemy.hitRing=.01}else if(this.lockedEnemy.health=Math.max(0,this.lockedEnemy.health-e/10),this.lockedEnemy.health<=0){let e=this.enemies.indexOf(this.lockedEnemy);e!==-1&&this.enemies.splice(e,1),this.lockedEnemy=null,this.cameraLocked=!1,this.orbitActive=!1,this.targetHud.clearLock()}}};if(this.lockedEnemy){let e=this.lockedEnemy;ye(this.lasers,n,e.pos,V),xe(this.pulses,n,e.pos,V)}if(this.laserRenderer.draw(this.lasers,D,N),this.laserRenderer.draw(this.pulses,D,N,this.laserRenderer.pulseColor,this.laserRenderer.pulseLength,this.laserRenderer.pulseWidth),this.shipState.lance.pendingFire&&!this.shipState.lance.reloading&&this.lanceTimeLeft<=0&&(this.shipState.lance.pendingFire=!1,this.cameraLocked&&this.fireLance()),this.lanceTimeLeft>0&&this.lockedEnemy){this.lanceTimeLeft-=n;let e=y((Math.random()-.5)*6,(Math.random()-.5)*2,(Math.random()-.5)*6);O(this.lanceEndOffset,this.lanceEndOffset,e,.18);let t=S(g(),this.lockedEnemy.pos,this.lanceEndOffset);this.beamRenderer.draw(D,N,this.shipPos,t,this.smoothCamPos,this.lanceTimeLeft,this.LANCE_DURATION),this.lanceTimeLeft<=0&&(this.shipState.lance.reloading=!0,this.shipState.lance.reloadTimer=this.shipState.lance.reloadTime)}let H=this.currentSpeed/Z,U=25-H*15,W=H*8;this.shipState.energy=Math.min(this.shipState.maxEnergy,this.shipState.energy+(U-W)*n);let G=5*(this.shipState.energy/this.shipState.maxEnergy);this.shipState.shields<this.shipState.maxShields&&(this.shipState.energy=Math.max(0,this.shipState.energy-4*n)),this.shipState.shields=Math.min(this.shipState.maxShields,this.shipState.shields+G*n);let K=Math.floor(this.shipState.shields/20);K>this.prevPlayerBubble&&(this.shipState.chargeWave=0),this.prevPlayerBubble=K,this.shipState.chargeWave>=0&&(this.shipState.chargeWave+=n*2.5,this.shipState.chargeWave>=1&&(this.shipState.chargeWave=-1)),this.shipState.hitRing>0&&(this.shipState.hitRing+=n*2.5,this.shipState.hitRing>=1&&(this.shipState.hitRing=0));for(let e of this.enemies){e.update(n,this.shipPos);for(let t of e.lasers)if(!t.dead&&T(t.pos,this.shipPos)<2.5)if(t.dead=!0,this.shipState.shields>=20){this.shipState.shields=Math.max(0,this.shipState.shields-10);let e=k(g(),t.pos,this.shipPos);E(this.shipState.hitPos,e),this.shipState.hitRing=.01}else this.shipState.health=Math.max(0,this.shipState.health-1);this.laserRenderer.draw(e.lasers,D,N,y(1,.3,.1)),this.ship.draw(e.modelMatrix,D,N,this.lightPos,this.smoothCamPos);let t=i();s(t,t,e.pos),c(t,t,[1.5,1.5,1.5]),this.shield.draw(t,D,N,this.smoothCamPos,e.shields,this.enemyShieldColor,e.hitRing,e.hitPos,e.chargeWave)}this.targetHud.update(this.enemies.map(e=>({obj:e,shields:e.shields,maxShields:e.maxShields,health:e.health,maxHealth:e.maxHealth,dist:T(this.shipPos,e.pos)}))),this.playerStatus.update(),this.hud.update(this.shipState,this.currentSpeed/Z*100),this.updateTargetIndicator(D,N),this.wasOrbiting=this.input.isOrbiting()}createTargetIndicator(){let e=document.createElement(`div`);e.id=`target-indicator`,e.style.cssText=`
      position: fixed; pointer-events: none; display: none;
      width: 0; height: 0;
      border-left: 8px solid transparent;
      border-right: 8px solid transparent;
      border-top: 14px solid rgba(255, 60, 60, 0.7);
      transform: translateX(-50%);
      filter: drop-shadow(0 0 4px rgba(255,60,60,0.5));
    `,document.body.appendChild(e),this.targetIndicator=e}updateTargetIndicator(e,t){if(!this.cameraLocked||!this.lockedEnemy){this.targetIndicator.style.display=`none`;return}let n=y(this.lockedEnemy.pos[0],this.lockedEnemy.pos[1]+3,this.lockedEnemy.pos[2]),r=h(i(),t,e),a=M(A(),j(n[0],n[1],n[2],1),r);if(a[3]<=0){this.targetIndicator.style.display=`none`;return}let o=a[0]/a[3],s=-a[1]/a[3],c=(o+1)/2*this.width,l=(s+1)/2*this.height;this.targetIndicator.style.display=`block`,this.targetIndicator.style.left=`${c}px`,this.targetIndicator.style.top=`${l}px`}fireBurst(){!this.cameraLocked||this.shipState.lasers.reloading||this.burstShotsLeft>0||(this.shipState.lasers.pendingFire=!1,this.burstShotsLeft=this.shipState.lasers.shotsPerBurst,this.burstCooldown=0)}firePulseBurst(){!this.cameraLocked||this.shipState.pulse.reloading||this.pulseShotsLeft>0||(this.shipState.pulse.pendingFire=!1,this.pulseShotsLeft=this.shipState.pulse.shotsPerBurst,this.pulseCooldown=0)}fireLance(){if(!(!this.cameraLocked||this.shipState.lance.reloading||this.lanceTimeLeft>0||!this.lockedEnemy)&&(this.shipState.lance.pendingFire=!1,this.shipState.energy=Math.max(0,this.shipState.energy-15),this.lanceTimeLeft=this.LANCE_DURATION,x(this.lanceEndOffset,0,0,0),!(this.lockedEnemy.shields>=20)&&(this.lockedEnemy.health=Math.max(0,this.lockedEnemy.health-3),this.lockedEnemy.health<=0))){let e=this.enemies.indexOf(this.lockedEnemy);e!==-1&&this.enemies.splice(e,1),this.lockedEnemy=null,this.cameraLocked=!1,this.orbitActive=!1,this.targetHud.clearLock()}}processInput(e){let t=this.input;if(this.shipState.engines=this.shipState.maxEngines,this.currentSpeed=this.shipState.engines/100*Z,this.orbitActive&&this.lockedEnemy){let t=this.yaw,n=k(g(),this.shipPos,this.lockedEnemy.pos);n[1]=0;let r=v(n);if(r>.01){let t=E(g(),n),i=E(g(),y(-t[2]+t[0]*(this.orbitRadius-r)*.02,0,t[0]+t[2]*(this.orbitRadius-r)*.02)),a=E(g(),y(this.shipFront[0],0,this.shipFront[2]));O(a,a,i,.04*e*60),E(a,a),this.shipFront[0]=a[0],this.shipFront[2]=a[2],this.shipFront[1]=0}this.pitch=0,this.pitchAngle+=(0-this.pitchAngle)*Ue*e*60,this.yaw=Math.atan2(this.shipFront[2],this.shipFront[0])*180/Math.PI;let i=((this.yaw-t)%360+540)%360-180,a=Math.max(-90,Math.min(Q,i*80*60));this.rollAngle+=(a-this.rollAngle)*Ve*e*60*2,w(this.shipPos,this.shipPos,this.shipFront,this.currentSpeed*e);return}let n=t.held(`KeyA`),i=t.held(`KeyD`),a=n?-90:i?Q:0;this.rollAngle+=(a-this.rollAngle)*Ve*e*60,this.rollAngle=Math.max(-90,Math.min(Q,this.rollAngle));let o=t.held(`KeyW`),s=t.held(`KeyS`);if(o||s){let t=Be*e*(o?-1:1),n=r(this.rollAngle);this.pitch+=t*Math.cos(n),this.yaw-=t*Math.sin(n),this.pitch=Math.max(-35,Math.min(He,this.pitch))}else this.pitch*=.88**(e*60);this.pitchAngle+=(this.pitch-this.pitchAngle)*Ue*e*60;let c=r(this.yaw),l=r(-this.pitch);this.shipFront=E(g(),y(Math.cos(c)*Math.cos(l),Math.sin(l),Math.sin(c)*Math.cos(l))),w(this.shipPos,this.shipPos,this.shipFront,this.currentSpeed*e)}},$=document.getElementById(`glCanvas`);$.width=window.innerWidth,$.height=window.innerHeight,window.addEventListener(`resize`,()=>{$.width=window.innerWidth,$.height=window.innerHeight,Ge.resize($.width,$.height)});var Ge=new We(e($),$.width,$.height,$);function Ke(){Ge.frame(),requestAnimationFrame(Ke)}requestAnimationFrame(Ke);